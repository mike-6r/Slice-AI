...
} from '../src/modules/identity/phone-verification/phone-verification.service';
...
const service = new PhoneVerificationService(
  Object.assign(db, {
    withTransaction: <T>(work: (tx: never) => Promise<T>) => db.$transaction(work as never),
  }) as never,
  config,
...
describe('phone verification PostgreSQL authority', () => {
  ...
  async function user(label: string) {
    const created = await db.user.create({
      data: {
        ...
        normalizedEmail: `${run}-${label}@example.test`,
  ...
  it('stores no OTP and permits exactly one concurrent confirmation', async () => {
    ...
    const results = await Promise.allSettled([
      service.confirm(fixture.actor, code, '198.51.100.2', `${run}-a`),
      service.confirm(fixture.actor, code, '198.51.100.3', `${run}-b`),
    ]);
  ...
  it('supersedes a resend and replaces a prior verified phone only on confirmation', async () => {
    const fixture = await user('change');
    const oldPhone = '+12025550101';
    const newPhone = '+12025550102';
    await service.send(fixture.actor, oldPhone, '198.51.100.4', `${run}-old`);
    await service.confirm(fixture.actor, delivery.codeForTest(fixture.user.id, oldPhone)!, '198.51.100.4', `${run}-old-confirm`);
    ...
    await expect(service.confirm(fixture.actor, firstCode, '198.51.100.7', `${run}-old-code`)).rejects.toMatchObject({ status: 401 });
    await expect(service.confirm(fixture.actor, delivery.codeForTest(fixture.user.id, newPhone)!, '198.51.100.7', `${run}-new-confirm`)).resolves.toMatchObject({ verified: true });
