...
const run = `provider-duplicate-reversal-${Date.now()}`;

describe('Document 016 duplicate provider reversal concurrency', () => {
  const userId = `${run}-user`;
  const cashAccountId = `${run}-cash`;
  const config = {
    providerMode: 'local', providersProductionEnabled: false,
    providerEncryptionKey: 'provider-duplicate-reversal-key-not-production',
  ...
  it('creates one compensating journal and one reversal transition under concurrent replay', async () => {
    const pending = await movements.createDeposit(actor, '700', `${run}-create`, `${run}-create-key`);
    await movements.completeFromProvider({ movementId: pending.id, providerReference: `${run}-provider-ref`, providerEventId: `${run}-event`, requestId: `${run}-complete` });
    const completed = await db.moneyMovement.findUniqueOrThrow({ where: { id: pending.id } });
    expect(completed.status).toBe('SETTLED');
    const original = await db.journalTransaction.findUniqueOrThrow({ where: { id: completed.ledgerTransactionId! }, include: { entries: { orderBy: { sequence: 'asc' } } } });
    const originalEntries = original.entries.map((entry) => ({ accountId: entry.accountId, side: entry.side, amountMinor: entry.amountMinor }));
    expect(await db.journalTransaction.count({ where: { correlationId: `provider-movement:${completed.id}` } })).toBe(1);
    expect(await db.journalTransaction.count({ where: { reversalOfId: original.id } })).toBe(0);
    expect(await db.moneyMovementHistory.count({ where: { movementId: completed.id, toStatus: 'REVERSED' } })).toBe(0);

    const results = await Promise.allSettled([
      movements.reverseFromProvider({ movementId: completed.id, reasonCode: 'RACE_REVERSAL', requestId: `${run}-a` }),
      movements.reverseFromProvider({ movementId: completed.id, reasonCode: 'RACE_REVERSAL', requestId: `${run}-b` }),
    ]);
    expect(results.some((result) => result.status === 'fulfilled')).toBe(true);

    const reversed = await db.moneyMovement.findUniqueOrThrow({ where: { id: completed.id } });
    expect(reversed.status).toBe('REVERSED');
    expect(await db.journalTransaction.count({ where: { correlationId: `provider-movement:${completed.id}` } })).toBe(1);
    expect(await db.journalTransaction.count({ where: { reversalOfId: original.id } })).toBe(1);
    expect(await db.moneyMovementHistory.count({ where: { movementId: completed.id, toStatus: 'REVERSED' } })).toBe(1);
    ...
    await movements.reverseFromProvider({ movementId: completed.id, reasonCode: 'REPLAY_REVERSAL', requestId: `${run}-replay` });
    expect(await db.journalTransaction.count({ where: { reversalOfId: original.id } })).toBe(1);
    expect(await db.moneyMovementHistory.count({ where: { movementId: completed.id, toStatus: 'REVERSED' } })).toBe(1);
