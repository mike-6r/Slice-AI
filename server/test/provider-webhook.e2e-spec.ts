...
describe('Document 016 provider webhook HTTP boundary', () => {
  ...
  afterAll(async () => {
    await h.db.webhookInbox.deleteMany({ where: { receivedAt: { gte: receivedAfter }, provider: 'BRIDGE' } });
  ...
  it('verifies exact raw bytes, persists unknown signed events, and deduplicates them', async () => {
    ...
    const first = await request(server).post('/api/v1/providers/BRIDGE/webhooks').set(headers).set('content-type', 'application/json').send(raw.toString());
    expect(first.status).toBe(201);
    expect(first.body).toEqual({ accepted: true, replayed: false });
    const replay = await request(server).post('/api/v1/providers/BRIDGE/webhooks').set(headers).set('content-type', 'application/json').send(raw.toString());
    expect(replay.status).toBe(201);
    expect(replay.body).toEqual({ accepted: true, replayed: true });
    expect(await h.db.webhookInbox.count({ where: { provider: 'BRIDGE', receivedAt: { gte: receivedAfter } } })).toBe(1);

    const tampered = Buffer.from(raw.toString().replace('unknown.event', 'unknown.tampered'));
    const invalid = await request(server).post('/api/v1/providers/BRIDGE/webhooks').set(headers).set('content-type', 'application/json').send(tampered.toString());
  ...
  it('rejects expired and invalid signatures before persisting an inbox event', async () => {
    ...
    const expired = await request(h.app.getHttpServer()).post('/api/v1/providers/BRIDGE/webhooks').set(signed(raw, Math.floor(Date.now() / 1000) - 600)).set('content-type', 'application/json').send(raw.toString());
    expect(expired.status).toBe(400);
    const bad = await request(h.app.getHttpServer()).post('/api/v1/providers/BRIDGE/webhooks').set({ 'x-provider-timestamp': String(Math.floor(Date.now() / 1000)), 'x-provider-signature': '00' }).set('content-type', 'application/json').send(raw.toString());
