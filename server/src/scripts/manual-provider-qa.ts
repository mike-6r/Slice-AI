...
if (!databaseUrl)
  throw new Error(
    'TEST_DATABASE_URL or DATABASE_URL is required for qa:providers.',
  );
  ...
  try {
    ...
    await db.complianceCase.create({
      data: {
        ...
        providerReferenceHash: crypto.hash(`${run}-case-ref`),
    ...
    await ledger.post(
      ...
      {
        type: 'DEMO_FUNDING',
        correlationId: `${run}-proceeds-funding`,
        descriptionCode: 'LOCAL_QA_PROCEEDS_FUNDING',
        lines: [
      ...
      `${run}-proceeds-funding-request`,
      `${run}-proceeds-funding-key`,
    );

    const deposit = await movements.createDeposit(
      actor,
      '10000',
      `${run}-deposit-create`,
      `${run}-deposit-key`,
    ...
    requireEqual(
      (await ledger.walletForUser(userId)).pendingMinor,
      '10000',
      'pending deposit',
    );
    requireEqual(
      (await ledger.walletForUser(userId)).accounts.find(
        (account) => account.code === 'CASH_AVAILABLE',
      )?.availableMinor,
      '0',
      'deposit remains pending',
    );
    await movements.completeFromProvider({
      movementId: deposit.id,
      providerReference: `${run}-deposit-ref`,
      providerEventId: `${run}-deposit-event`,
      requestId: `${run}-deposit-complete`,
    });
    const depositReplay = await movements.completeFromProvider({
      movementId: deposit.id,
      providerReference: `${run}-deposit-ref`,
      providerEventId: `${run}-deposit-event`,
      requestId: `${run}-deposit-replay`,
    });
    ...
    const proceedsWithdrawal = await movements.createWithdrawal(
      actor,
      '6000',
      `${run}-proceeds-withdrawal-create`,
      `${run}-proceeds-withdrawal-key`,
      'LOCAL_LOW_RISK',
    );
    requireEqual(
      (
        await db.moneyMovement.findUniqueOrThrow({
          where: { id: proceedsWithdrawal.id },
        })
      ).cashAccountId,
      proceedsAccountId,
      'collector proceeds withdrawal source',
    );
    await movements.completeFromProvider({
      movementId: proceedsWithdrawal.id,
      providerReference: `${run}-proceeds-withdrawal-ref`,
      providerEventId: `${run}-proceeds-withdrawal-event`,
      requestId: `${run}-proceeds-withdrawal-complete`,
    });
    requireEqual(
      (await ledger.walletForUser(userId)).collectorProceedsMinor,
      '4000',
      'collector proceeds after withdrawal',
    );

    const withdrawal = await movements.createWithdrawal(
      actor,
      '5000',
      `${run}-withdrawal-create`,
      `${run}-withdrawal-key`,
      'LOCAL_LOW_RISK',
    );
    requireEqual(
      ...
      cashAccountId,
      'cash fallback withdrawal source',
    );
    requireEqual(
      (
    ...
    await movements.completeFromProvider({
      ...
      providerReference: `${run}-withdrawal-ref`,
      providerEventId: `${run}-withdrawal-event`,
      requestId: `${run}-withdrawal-complete`,
    });
    requireEqual(
      (await ledger.walletForUser(userId)).accounts.find(
        (account) => account.code === 'CASH_AVAILABLE',
    ...
    const failed = await movements.createWithdrawal(
      actor,
      '500',
      `${run}-failed-create`,
      `${run}-failed-key`,
    ...
    await movements.failFromProvider({
      movementId: failed.id,
      reasonCode: 'LOCAL_QA_FAILED',
    ...
    const cancellable = await movements.createWithdrawal(
      actor,
      '500',
      `${run}-cancel-create`,
      `${run}-cancel-key`,
    ...
    await movements.cancelFromProvider({
      movementId: cancellable.id,
      reasonCode: 'LOCAL_QA_CANCELLED',
    ...
    requireEqual(
      ...
      'RELEASED',
      'cancelled withdrawal release',
    );

    const hold = await holds.create(actor, {
      userId,
      scope: 'WITHDRAWAL',
      reasonCode: 'LOCAL_QA_HOLD',
      requestId: `${run}-hold`,
    });
    await movements
      .createWithdrawal(
    ...
    const reversible = await movements.createDeposit(
      actor,
      '700',
      `${run}-reverse-create`,
      `${run}-reverse-key`,
    ...
    await movements.completeFromProvider({
      movementId: reversible.id,
      providerReference: `${run}-reverse-ref`,
      providerEventId: `${run}-reverse-event`,
      requestId: `${run}-reverse-complete`,
    });
    await movements.returnFromProvider({
      movementId: reversible.id,
      reasonCode: 'LOCAL_QA_RETURNED',
      requestId: `${run}-return`,
    });
    await movements.returnFromProvider({
      movementId: reversible.id,
      reasonCode: 'LOCAL_QA_RETURNED_REPLAY',
      requestId: `${run}-return-replay`,
    });
    requireEqual(
      (
        await db.moneyMovement.findUniqueOrThrow({
    ...
    const returnedRecord = await db.moneyMovement.findUniqueOrThrow({
      where: { id: reversible.id },
    });
    const returnReversal = await db.journalTransaction.findFirstOrThrow({
      where: { reversalOfId: returnedRecord.ledgerTransactionId! },
    });
    ...
    console.log(
      JSON.stringify({
        ...
        movementIds: {
          deposit: deposit.id,
          proceedsWithdrawal: proceedsWithdrawal.id,
          withdrawal: withdrawal.id,
          failedWithdrawal: failed.id,
          cancellation: cancellable.id,
          returnedDeposit: reversible.id,
        },
        returnJournalIds: {
          original: returnedRecord.ledgerTransactionId,
          reversal: returnReversal.id,
        },
        deposit: 'PENDING_THEN_SETTLED_ONCE',
        proceedsWithdrawal: 'SETTLED_ONCE',
        withdrawal: 'SETTLED_ONCE',
        failedWithdrawal: 'RELEASED_ONCE',
        cancellation: 'RELEASED_ONCE',
        hold: 'ENFORCED_AND_RELEASED',
        returned: 'APPEND_ONLY_AND_REPLAY_SAFE',
    ...
    await db.providerDiscrepancy.deleteMany({
      where: { run: { actorUserId: userId } },
    });
    await db.providerReconciliationRun.deleteMany({
    ...
    console.log(
      JSON.stringify({
        cleanup: {
          users: residual[0],
          movements: residual[1],
          reservations: residual[2],
          complianceCases: residual[3],
        },
      }),
    );
    await db.$disconnect();
