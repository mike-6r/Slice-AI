...
async function main() {
  ...
  try {
    ...
    await db.category.create({
      data: { id: categoryId, slug: categoryId, name: 'Manual trading QA' },
    });
    ...
    await db.ownershipAccount.create({
      data: { id: sellerOwnershipId, type: 'USER', userId: sellerId },
    });
    ...
    await ledger.post(
      actor(buyerId),
      {
        ...
        descriptionCode: 'MANUAL_TRADING_QA_FUNDING',
        lines: [
    ...
    await lots.recordAcquisition(
      actor(sellerId),
      {
        ...
        units: '10',
        totalCostMinor: '500',
        sourceReference: `${run}-seller-acquisition`,
      },
      `${run}-lot-request`,
      `${run}-lot-key`,
    );
    ...
    const sell = await trading.place(
      actor(sellerId),
      {
        assetId,
        side: 'SELL',
    ...
    const buy = await trading.place(
      actor(buyerId),
      buyInput,
      `${run}-buy-request`,
      `${run}-buy-key`,
    );
    const replay = await trading.place(
      actor(buyerId),
      buyInput,
      `${run}-buy-request`,
      `${run}-buy-key`,
    ...
    assert(
      buy.id === replay.id && buy.status === 'FILLED',
      'buy replay or matching was not stable.',
    );
    const execution = await db.tradingExecution.findFirstOrThrow({
      where: { buyOrderId: buy.id, sellOrderId: sell.id },
    });
    assert(
      execution.priceMinor === 100n && execution.units === 4n,
      'maker-price execution was incorrect.',
    );
    assert(
      execution.buyerFeeMinor === 4n && execution.sellerFeeMinor === 0n,
      'configured 0/100 bps fees were not applied.',
    );
    assert(
      (
        await db.accountBalance.findUniqueOrThrow({
          where: { accountId: buyerCashId },
        })
      ).reservedMinor === 0n,
      'filled buy did not consume its cash reservation.',
    );
    const buyerOwnership = await db.ownershipAccount.findUniqueOrThrow({
      where: { userId: buyerId },
    });
    assert(
      (
        await db.ownershipPosition.findUniqueOrThrow({
          where: {
            assetId_accountId: { assetId, accountId: buyerOwnership.id },
          },
        })
      ).settledUnits === 4n,
      'buyer ownership did not settle.',
    );
    await expectCode(
      () =>
        trading.place(
          ...
          `${run}-insufficient-request`,
          `${run}-insufficient-key`,
        ),
      'INSUFFICIENT_FUNDS',
    );
    const open = await trading.place(
      actor(buyerId),
      {
        assetId,
        side: 'BUY',
    ...
    const cancelled = await trading.cancel(
      actor(buyerId),
      open.id,
      `${run}-cancel-action`,
      `${run}-cancel-action-key`,
    );
    assert(
      cancelled.status === 'CANCELLED',
      'cancellation did not close the open remainder.',
    );
    const book = await trading.publicBook(assetId, 10);
    assert(
      JSON.stringify(book).match(/buyer|seller|account|reservation/i) === null,
      'public book leaked private data.',
    );
    const history = await trading.ownExecutions(buyerId, undefined, 10);
    assert(
      history.items.length === 1 && history.items[0].feeMinor === '4',
      'safe execution history was incomplete.',
    );
    ...
    const executions = await db.tradingExecution
      .findMany({
        where: { assetId },
        select: { buyerFeeMinor: true, sellerFeeMinor: true },
      })
      .catch(() => []);
    const totalFees = executions.reduce(
      (sum, item) => sum + item.buyerFeeMinor + item.sellerFeeMinor,
      0n,
    );
    const orderIds = (
      await db.tradingOrder
        .findMany({ where: { assetId }, select: { id: true } })
        .catch(() => [])
    ).map((row) => row.id);
    const journalIds = (
      await db.journalTransaction
        .findMany({
          where: {
            OR: [
              { correlationId: { startsWith: `trade:${assetId}` } },
              { correlationId: `${run}-funding` },
            ],
          },
          select: { id: true },
    ...
    await db.auditEvent
      .deleteMany({ where: { actorUserId: { in: [buyerId, sellerId] } } })
      .catch(() => undefined);
    await db.idempotencyRecord
      .deleteMany({
        where: { actorScope: { in: [`user:${buyerId}`, `user:${sellerId}`] } },
      })
      .catch(() => undefined);
    await db.orderStatusHistory
      .deleteMany({ where: { orderId: { in: orderIds } } })
      .catch(() => undefined);
    await db.tradingExecution
      .deleteMany({ where: { assetId } })
      .catch(() => undefined);
    await db.tradingOrder
      .deleteMany({ where: { id: { in: orderIds } } })
      .catch(() => undefined);
    await db.tradingMarket
      .deleteMany({ where: { assetId } })
      .catch(() => undefined);
    await db.journalEntry
      .deleteMany({ where: { transactionId: { in: journalIds } } })
      .catch(() => undefined);
    await db.journalTransaction
      .deleteMany({ where: { id: { in: journalIds } } })
      .catch(() => undefined);
    if (totalFees > 0n) {
      const feeAccount = await db.financialAccount
        .findFirst({
          where: {
            ownerType: 'PLATFORM',
      ...
      if (feeAccount)
        await db.accountBalance
          .update({
            ...
            data: { postedCreditMinor: { decrement: totalFees } },
          })
          .catch(() => undefined);
    }
    await db.lotDisposal
      .deleteMany({ where: { lot: { userId: { in: [buyerId, sellerId] } } } })
      .catch(() => undefined);
    await db.portfolioLot
      .deleteMany({ where: { userId: { in: [buyerId, sellerId] } } })
      .catch(() => undefined);
    await db.cashReservation
      .deleteMany({
        where: { account: { ownerUserId: { in: [buyerId, sellerId] } } },
      })
      .catch(() => undefined);
    const ownershipIds = (
    ...
    await db.ownershipPosition
      .deleteMany({ where: { accountId: { in: ownershipIds } } })
      .catch(() => undefined);
    await db.ownershipAccount
      .deleteMany({ where: { id: { in: ownershipIds } } })
      .catch(() => undefined);
    const financialIds = (
      await db.financialAccount
        .findMany({
          where: { ownerUserId: { in: [buyerId, sellerId] } },
          select: { id: true },
        })
        .catch(() => [])
    ).map((row) => row.id);
    await db.accountBalance
      ...
      .catch(() => undefined);
    await db.financialAccount
      .deleteMany({ where: { id: { in: [...financialIds, clearingId] } } })
      .catch(() => undefined);
    await db.ownershipAssetSupply
      .deleteMany({ where: { assetId } })
      .catch(() => undefined);
    await db.asset
      .deleteMany({ where: { id: assetId } })
      .catch(() => undefined);
    await db.category
      .deleteMany({ where: { id: categoryId } })
      .catch(() => undefined);
    await db.user
      .deleteMany({ where: { id: { in: [buyerId, sellerId] } } })
      .catch(() => undefined);
    const [
      orders,
      ...
      db.financialAccount.count({
        where: {
          OR: [
            { ownerUserId: { in: [buyerId, sellerId] } },
            { id: clearingId },
          ],
        },
      }),
    ...
    console.log(
      JSON.stringify({
        ...
        cleanup: {
          orders,
          executions: executionsAfterCleanup,
          cashReservations,
          lots: lotsAfterCleanup,
          financialAccounts,
          journalTransactions,
          ownershipAccounts,
          ownershipPositions,
