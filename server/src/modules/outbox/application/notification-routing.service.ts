...
export const notificationTopic = {
  ...
  collectorActions: 'COLLECTOR_ACTIONS',
  shipping: 'SHIPPING',
  securityAlerts: 'SECURITY_ALERTS',
  financialAlerts: 'FINANCIAL_ALERTS',
} as const;
...
export class NotificationRoutingService {
  ...
  route(event: OutboxEvent): DeliveryIntent[] {
    if (
      event.eventType === eventType.tradeCompleted &&
      event.schemaVersion === 1
    )
      return this.publicTrade(event);
    if (
      [
      ...
      return this.privateOrder(event);
    if (
      [
      ...
      return this.privateResource(event, notificationTopic.collectorActions);
    if (
      [
      ...
      return this.privateResource(event, notificationTopic.shipping);
    if (
      event.eventType === eventType.movementSettled &&
      event.schemaVersion === 1
    )
      return this.privateMovement(event);
    if (
      event.eventType === eventType.financialNotification &&
      event.schemaVersion === 1
    )
      return this.privateFinancial(event);
  ...
  private publicTrade(event: OutboxEvent): DeliveryIntent[] {
    ...
    return [
      {
        channel: 'DISCORD',
        destinationKey: 'discord.market_feed',
        classification: 'PUBLIC',
        topic: notificationTopic.marketListings,
        mandatory: false,
        payloadVersion: 1,
        payload: {
  ...
  private privateOrder(event: OutboxEvent): DeliveryIntent[] {
    ...
    if (!event.actorUserId || !isOrderPayload(payload))
      throw new OutboxHandlerError('NON_RETRYABLE', 'EVENT_PAYLOAD_INVALID');
    const order = {
      eventId: event.eventId,
      ...
      ...(payload.priceMinor ? { priceMinor: payload.priceMinor } : {}),
      occurredAt: event.occurredAt.toISOString(),
    };
    return [
      {
        channel: 'IN_APP',
        destinationKey: `user:${event.actorUserId}`,
        classification: 'PRIVATE',
        topic: notificationTopic.orderUpdates,
        mandatory: false,
        payloadVersion: 1,
        payload: order,
      },
  ...
  private privateMovement(event: OutboxEvent): DeliveryIntent[] {
    ...
    if (!event.actorUserId || !isMovementPayload(payload))
      throw new OutboxHandlerError('NON_RETRYABLE', 'EVENT_PAYLOAD_INVALID');
    return [
      {
        channel: 'IN_APP',
        destinationKey: `user:${event.actorUserId}`,
        classification: 'PRIVATE',
        topic: notificationTopic.portfolioUpdates,
        mandatory: false,
        payloadVersion: 1,
        payload: {
          eventId: event.eventId,
  ...
  private privateFinancial(event: OutboxEvent): DeliveryIntent[] {
    ...
    if (!event.actorUserId || !isFinancialPayload(payload))
      throw new OutboxHandlerError('NON_RETRYABLE', 'EVENT_PAYLOAD_INVALID');
    const value = {
      eventId: event.eventId,
      kind: payload.kind,
      ...
      ...(payload.amountMinor === undefined
        ? {}
        : { amountMinor: payload.amountMinor }),
      ...(payload.outstandingMinor === undefined
        ? {}
        : { outstandingMinor: payload.outstandingMinor }),
      occurredAt: event.occurredAt.toISOString(),
    };
    return [
      {
        ...
        destinationKey: `user:${event.actorUserId}`,
        classification: 'PRIVATE',
        topic: notificationTopic.financialAlerts,
        mandatory: true,
        payloadVersion: 1,
        payload: value,
      },
  ...
  private privateResource(event: OutboxEvent, topic: string): DeliveryIntent[] {
    ...
    if (!event.actorUserId || !isResourcePayload(payload))
      throw new OutboxHandlerError('NON_RETRYABLE', 'EVENT_PAYLOAD_INVALID');
    const value = {
      eventId: event.eventId,
      eventType: event.eventType,
      submissionId: payload.submissionId,
      ...(payload.intakeId ? { intakeId: payload.intakeId } : {}),
      status: payload.status,
      ...(payload.requestedItems?.length
  ...
  optionalUserIntent(input: {
    userId: string;
    topic: string;
    channel: NotificationDeliveryChannel;
    enabled: boolean;
    mandatory: boolean;
    payload: DeliveryIntent['payload'];
  }): DeliveryIntent | null {
    if (!input.mandatory && !input.enabled) return null;
    return {
      channel: input.channel,
      destinationKey: `user:${input.userId}`,
      classification: 'PRIVATE',
      topic: input.topic,
...
function isTradePayload(
  payload: Partial<TradeCompletedPayload>,
): payload is TradeCompletedPayload {
  return (
...
): payload is OrderLifecyclePayload {
  return (
...
function isResourcePayload(
  payload: Partial<CustomerResourcePayload>,
): payload is CustomerResourcePayload {
  return (
    typeof payload.submissionId === 'string' &&
    typeof payload.status === 'string' &&
    (payload.intakeId === undefined || typeof payload.intakeId === 'string') &&
    (payload.requestedItems === undefined ||
      (Array.isArray(payload.requestedItems) &&
        payload.requestedItems.every((item) => typeof item === 'string'))) &&
    (payload.customerMessage === undefined ||
      typeof payload.customerMessage === 'string')
  );
}
function isMovementPayload(
  payload: Partial<MovementSettledPayload>,
): payload is MovementSettledPayload {
  return (
    typeof payload.movementId === 'string' &&
    (payload.type === 'DEPOSIT' || payload.type === 'WITHDRAWAL') &&
...
function isFinancialPayload(
  payload: Partial<FinancialNotificationPayload>,
): payload is FinancialNotificationPayload {
  return (
    ...
    (payload.amountMinor === undefined ||
      typeof payload.amountMinor === 'string') &&
    (payload.outstandingMinor === undefined ||
      typeof payload.outstandingMinor === 'string')
  );
}
