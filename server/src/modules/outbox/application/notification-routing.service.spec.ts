...
describe('NotificationRoutingService private order delivery', () => {
  ...
  it('centralizes Collector Actions and Shipping category mapping', () => {
    ...
    const submission = routing.route({ id: 'row-2', eventId: 'submission.approved:submission-1', eventType: 'submission.approved', schemaVersion: 1, actorUserId: 'user-1', occurredAt: new Date(), payload: { submissionId: 'submission-1', status: 'APPROVED' } } as never);
    const receipt = routing.route({ id: 'row-3', eventId: 'intake.receiptconfirmed:submission-1', eventType: 'intake.receiptconfirmed', schemaVersion: 1, actorUserId: 'user-1', occurredAt: new Date(), payload: { submissionId: 'submission-1', intakeId: 'intake-1', status: 'RECEIVED' } } as never);
    expect(submission).toEqual(expect.arrayContaining([expect.objectContaining({ channel: 'DISCORD', topic: 'COLLECTOR_ACTIONS' })]));
    expect(receipt).toEqual(expect.arrayContaining([expect.objectContaining({ channel: 'DISCORD', topic: 'SHIPPING' })]));
  });
});
