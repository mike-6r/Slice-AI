...
export class NotificationDeliveryWorkerRepository {
  ...
  async claimBatch(workerId: string, batchSize: number, leaseMs: number, now: Date): Promise<DeliveryClaim[]> {
    return this.db.$transaction(async (tx) => {
      // Discord is claimed by the authenticated bot consumer, never by the
      // in-process transport registry. This preserves one durable queue while
      // preventing an unimplemented local transport from dead-lettering it.
      const ids = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`SELECT "id" FROM "NotificationDelivery" WHERE "channel" <> 'DISCORD'::"NotificationDeliveryChannel" AND (("status"='PENDING'::"NotificationDeliveryStatus" AND "availableAt"<=${now}) OR ("status"='PROCESSING'::"NotificationDeliveryStatus" AND "leaseExpiresAt"<=${now})) ORDER BY "availableAt","createdAt","id" LIMIT ${Math.min(Math.max(batchSize,1),100)} FOR UPDATE SKIP LOCKED`);
  ...
  async claimDiscordPrivateBatch(workerId: string, batchSize: number, leaseMs: number, now: Date): Promise<DeliveryClaim[]> {
    return this.db.$transaction(async (tx) => {
      const ids = await tx.$queryRaw<Array<{ id: string }>>(Prisma.sql`SELECT "id" FROM "NotificationDelivery" WHERE "channel"='DISCORD'::"NotificationDeliveryChannel" AND "classification"='PRIVATE' AND (("status"='PENDING'::"NotificationDeliveryStatus" AND "availableAt"<=${now}) OR ("status"='PROCESSING'::"NotificationDeliveryStatus" AND "leaseExpiresAt"<=${now})) ORDER BY "availableAt","createdAt","id" LIMIT ${Math.min(Math.max(batchSize,1),100)} FOR UPDATE SKIP LOCKED`);
      const tokens = new Map<string, string>(); const expires = new Date(now.getTime() + leaseMs);
      for (const { id } of ids) { const claimToken = randomUUID(); tokens.set(id, claimToken); await tx.notificationDelivery.update({ where: { id }, data: { status: 'PROCESSING', lockedBy: workerId, lockedAt: now, claimToken, leaseExpiresAt: expires, attempts: { increment: 1 }, lastAttemptAt: now } }); }
      const rows = await tx.notificationDelivery.findMany({ where: { id: { in: ids.map((row) => row.id) } } }); const byId = new Map(rows.map((row) => [row.id, row]));
      return ids.map(({ id }) => ({ ...byId.get(id)!, claimToken: tokens.get(id)! }));
    });
  }
  ...
  async suppress(id: string, token: string, now: Date, code: string) { return (await this.db.notificationDelivery.updateMany({ where: { id, status: 'PROCESSING', claimToken: token }, data: { status: 'SUPPRESSED', lastErrorSafe: code, lockedAt: null, lockedBy: null, claimToken: null, leaseExpiresAt: null, failedAt: now } })).count === 1; }
}
