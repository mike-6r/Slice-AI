...
export class OwnershipPolicyService {
  ...
  async adminProjection(assetId: string) {
    ...
    const blockers = [
      ...
      status !== 'APPROVED' && status !== 'ISSUED'
        ...
        : null,
      asset.operationalControl?.status === 'FROZEN'
    ...
    return {
      ...
      policy: {
        ...
        candidates: STANDARD_OWNERSHIP_POLICY.candidates.map((value) =>
          value.toString(),
        ),
  ...
  propose(
    ...
    if (input.policyCode !== STANDARD_OWNERSHIP_POLICY.code)
      throw new ConflictException({
        code: 'SUPPLY_POLICY_UNSUPPORTED',
        message: 'That supply policy is not available.',
      });
    return this.mutate(
      actor,
      assetId,
      ...
      async (db, audit) => {
        ...
        if (asset.ownershipSupply)
          throw new ConflictException({
            ...
            message: 'Supply cannot be configured after issuance.',
          });
        if (
          asset.ownershipSupplyPolicy &&
          asset.ownershipSupplyPolicy.status !== 'REJECTED'
        )
          throw new ConflictException({
            code: 'SUPPLY_POLICY_ALREADY_CONFIGURED',
            message: 'A supply proposal already exists for this asset.',
          });
        const decision = asset.valuationDecisions[0];
        if (!decision)
          throw new ConflictException({
            code: 'VALUATION_REQUIRED',
            message: 'An active valuation is required before proposing supply.',
          });
        ...
        const policy = await db.ownershipSupplyPolicy.upsert({
          ...
          create: {
            ...
            valuationCurrency: decision.currency,
            pricePerUnitMinor: BigInt(preview.pricePerUnitMinor),
            remainderMinor: BigInt(preview.remainderMinor),
        ...
        await audit('OWNERSHIP_SUPPLY_PROPOSED', {
          ...
          valuationMinor: decision.valueMinor.toString(),
          valuationCurrency: decision.currency,
          pricePerUnitMinor: preview.pricePerUnitMinor,
          remainderMinor: preview.remainderMinor,
          reason: input.reason.trim(),
        });
        return {
          assetId,
  ...
  approve(
    ...
    return this.mutate(
      ...
      async (db, audit) => {
        ...
        if (!policy)
          throw new NotFoundException({
            code: 'SUPPLY_POLICY_NOT_FOUND',
            message: 'Propose a supply policy before approval.',
          });
        ...
        if (control?.status === 'FROZEN')
          throw new ConflictException({
            code: 'ASSET_OPERATIONS_FROZEN',
            message:
              'Ownership approval is blocked while asset operations are frozen.',
          });
        if (policy.status === 'ISSUED')
          throw new ConflictException({
            code: 'OWNERSHIP_ALREADY_ISSUED',
            message: 'Issued supply cannot be approved again.',
          });
        if (policy.status !== 'PROPOSED')
          throw new ConflictException({
            code: 'SUPPLY_POLICY_NOT_PROPOSED',
            message: 'Only a proposed supply policy can be approved.',
          });
        const decision = await db.valuationDecision.findFirst({
        ...
        if (
          ...
          decision.currency !== policy.valuationCurrency
        )
          throw new ConflictException({
            code: 'VALUATION_CHANGED',
            message:
              'The authoritative valuation changed. Propose the supply again.',
          });
        const updated = await db.ownershipSupplyPolicy.update({
          where: { assetId },
          data: {
            status: 'APPROVED',
        ...
        await audit('OWNERSHIP_SUPPLY_APPROVED', {
          ...
          totalUnits: updated.proposedUnits.toString(),
          valuationMinor: updated.valuationMinor.toString(),
          valuationCurrency: updated.valuationCurrency,
          pricePerUnitMinor: updated.pricePerUnitMinor.toString(),
          remainderMinor: updated.remainderMinor.toString(),
          reason: reason.trim(),
        });
        return {
          assetId,
          status: updated.status,
          units: updated.proposedUnits.toString(),
          pricePerUnitMinor: updated.pricePerUnitMinor.toString(),
          remainderMinor: updated.remainderMinor.toString(),
        };
      },
  ...
  private async mutate<T extends Record<string, unknown>>(
    ...
    const identity: IdempotencyIdentity = {
      ...
      scope: `ownership.supply-policy.${operation}:${assetId}`,
      key,
    };
    ...
    return this.db.$transaction(async (db) => {
      ...
      if (acquired.state === 'FINGERPRINT_CONFLICT')
        throw new ConflictException({
          ...
          message:
            'The request key cannot be reused for another policy change.',
        });
