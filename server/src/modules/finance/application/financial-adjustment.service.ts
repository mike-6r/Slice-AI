...
export class FinancialAdjustmentService {
  ...
  async list(actor: Actor, status?: string) {
    ...
    return {
      ...
      pagination: { total: rows.length, limit: 100 },
    };
  }
  ...
  async create(
    ...
    if (!input.reason.trim() || input.reason.trim().length < 12)
      throw new ConflictException({
        code: 'FINANCIAL_ADJUSTMENT_REASON_REQUIRED',
    ...
    const requestHash = this.hash(
      JSON.stringify({ ...input, amountMinor: amountMinor.toString() }),
    );
    return this.db.$transaction(async (db) => {
      const existing = await db.financialAdjustmentRequest.findUnique({
      ...
      if (existing) {
        if (existing.requestHash !== requestHash)
          throw new ConflictException({
            ...
            message:
              'The request key cannot be reused for a different adjustment.',
          });
        return { ...this.safe(existing), replayed: true };
      }
      const deficit = await db.financialDeficit.findUnique({
        where: { id: input.deficitId },
      });
      if (!deficit || deficit.userId !== input.userId)
        throw new NotFoundException({
          code: 'FINANCIAL_DEFICIT_NOT_FOUND',
          message: 'The selected outstanding balance was not found.',
        });
      const outstanding = deficit.amountMinor - deficit.recoveredMinor;
      if (
        deficit.currency !== 'GBP' ||
        !['OPEN', 'PARTIALLY_RECOVERED'].includes(deficit.status) ||
        amountMinor <= 0n ||
        amountMinor > outstanding
      )
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_AMOUNT_INVALID',
          message:
            'The adjustment must be a positive GBP amount no greater than the outstanding balance.',
        });
  ...
  async submit(
    ...
    return this.db.$transaction(async (db) => {
      ...
      if (row.submissionIdempotencyKeyHash === keyHash)
        return { ...this.safe(row), replayed: true };
      if (row.status !== 'DRAFT')
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_NOT_DRAFT',
          message: 'Only a draft adjustment can be submitted.',
        });
      if (row.initiatorUserId !== actor.userId)
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_INITIATOR_REQUIRED',
          message: 'Only the finance initiator can submit this adjustment.',
        });
      const updated = await db.financialAdjustmentRequest.update({
        where: { id },
        data: {
          status: 'PENDING_APPROVAL',
          submissionIdempotencyKeyHash: keyHash,
        },
      });
      await this.audit(
        db,
  ...
  async approve(
    ...
    return this.db.$transaction(async (db) => {
      const row = await this.lock(db, id);
      if (row.approvalIdempotencyKeyHash === approvalHash)
        return { ...this.safe(row), replayed: true };
      if (row.initiatorUserId === actor.userId)
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_SECOND_APPROVER_REQUIRED',
          message:
            'A second authorized finance operator must approve this adjustment.',
        });
      if (row.status !== 'PENDING_APPROVAL')
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_NOT_PENDING',
          message: 'Only a submitted adjustment can be approved.',
        });

      await db.$queryRaw`SELECT id FROM "FinancialDeficit" WHERE id = ${row.deficitId} FOR UPDATE`;
      const deficit = await db.financialDeficit.findUnique({
        where: { id: row.deficitId },
      });
      if (
        !deficit ||
        deficit.userId !== row.userId ||
        deficit.currency !== 'GBP' ||
        !['OPEN', 'PARTIALLY_RECOVERED'].includes(deficit.status)
      )
        throw new ConflictException({
          code: 'FINANCIAL_DEFICIT_NOT_AVAILABLE',
          message:
            'The outstanding balance is no longer available for adjustment.',
        });
      ...
      if (row.amountMinor <= 0n || row.amountMinor > beforeOutstanding)
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_AMOUNT_INVALID',
          message: 'The adjustment exceeds the current outstanding balance.',
        });
  ...
  async reject(
    ...
    if (reason.trim().length < 12)
      throw new ConflictException({
        code: 'FINANCIAL_ADJUSTMENT_REJECTION_REASON_REQUIRED',
        message: 'A clear rejection reason is required.',
      });
    const rejectionHash = this.hash(idempotencyKey);
    return this.db.$transaction(async (db) => {
      const row = await this.lock(db, id);
      if (row.approvalIdempotencyKeyHash === rejectionHash)
        return { ...this.safe(row), replayed: true };
      if (row.initiatorUserId === actor.userId)
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_SECOND_REVIEWER_REQUIRED',
          message:
            'A second authorized finance operator must review this adjustment.',
        });
      if (row.status !== 'PENDING_APPROVAL')
        throw new ConflictException({
          code: 'FINANCIAL_ADJUSTMENT_NOT_PENDING',
          message: 'Only a submitted adjustment can be rejected.',
        });
      const updated = await db.financialAdjustmentRequest.update({
        where: { id },
        data: {
          status: 'REJECTED',
          approverUserId: actor.userId,
          approvalIdempotencyKeyHash: rejectionHash,
          rejectedAt: new Date(),
  ...
  private async lock(db: Db, id: string) {
    ...
    const row = await db.financialAdjustmentRequest.findUnique({
      where: { id },
    });
    if (!row)
      throw new NotFoundException({
        code: 'FINANCIAL_ADJUSTMENT_NOT_FOUND',
        message: 'The finance adjustment was not found.',
      });
    return row;
  }
  ...
  private async audit(
    ...
    await createIdentityTransaction(db).audit.append({
      ...
      actorType: 'USER',
      action,
      resourceType: 'financial-adjustment',
      resourceId,
      requestId,
  ...
  private safe(row: {
    ...
    amountMinor: bigint;
    reason: string;
    beforeOutstandingMinor: bigint;
    afterOutstandingMinor: bigint | null;
    restrictionReleased: boolean | null;
    requestedAt: Date;
