...
import {
  ...
  AI_REQUIRED_MEDIA_SLOTS,
} from '../domain/submission.policy';
import {
  RAW_CARD_PREGRADE_PROVIDER,
  type RawCardPreGradeProvider,
  type RawCardVisualization,
} from './raw-card-pregrade.provider';
...
export class RawCardPreGradeService {
  ...
  async getOwned(actor: Actor, submissionId: string) {
    ...
    const current =
      submission.preGrades.find((item) => !item.supersededAt) ?? null;
    return {
      current: current ? await this.decorateProjection(current) : null,
      history: await Promise.all(
        submission.preGrades.map((item) => this.decorateProjection(item)),
      ),
    };
  }
  ...
  async analyze(actor: Actor, submissionId: string, requestId: string) {
    ...
    const media = AI_REQUIRED_MEDIA_SLOTS.map((slot) =>
      submission.media.find(
        (item) => item.slot === slot && item.status === 'SAFE',
      ),
    );
    ...
    if (cached?.status === 'SUCCEEDED') {
      const existingVisualizations = Array.isArray(cached.visualizations)
        ? cached.visualizations
        : [];
      if (
        !existingVisualizations.length &&
        cached.rawResponse &&
        this.provider.extractVisualizations
      ) {
        const sourceVisualizations = this.provider.extractVisualizations(
          cached.rawResponse,
        );
        const persistedVisualizations = await this.copyVisualizations(
          submission.id,
          cached.id,
          front.id,
          back.id,
        ...
        if (persistedVisualizations.length) {
          const repaired = await this.db.rawCardPreGrade.update({
            where: { id: cached.id },
            data: {
              visualizations: persistedVisualizations as Prisma.InputJsonValue,
            },
          });
    ...
    if (!analysis) {
      try {
        ...
        analysis = await this.db.rawCardPreGrade.findUniqueOrThrow({
          where: {
            submissionId_analysisFingerprint: {
              submissionId: submission.id,
              analysisFingerprint: fingerprint,
            },
          },
        ...
        if (analysis.status === 'IN_PROGRESS')
          return this.decorateProjection(analysis);
      }
    ...
    await this.audit(
      actor.userId,
      'RAW_CARD_PREGRADE_REQUESTED',
      submission.id,
      requestId,
      { analysisId: analysis.id, provider: this.provider.providerName },
    );
  ...
  private async runProvider(
    ...
    const [front, back] = await Promise.all([
      this.storage.read(frontKey),
      this.storage.read(backKey),
    ]);
    if (!front || !back)
      throw new ServiceUnavailableException({
        code: 'STORAGE_UNAVAILABLE',
        message: 'The card photos could not be read for analysis.',
      });
    ...
    const persistedVisualizations =
      result.status === 'SUCCEEDED'
        ? await this.copyVisualizations(
            submissionId,
            analysisId,
            frontMediaId,
            backMediaId,
    ...
    await this.audit(
      actor.userId,
      result.status === 'SUCCEEDED'
        ? 'RAW_CARD_PREGRADE_COMPLETED'
        : 'RAW_CARD_PREGRADE_FAILED',
      submissionId,
      requestId,
      {
        analysisId,
  ...
  private async persist(
    ...
    const data = {
      ...
      providerRequestId:
        (result.providerRequestId as string | null | undefined) ?? null,
      status,
      ...
      rawResponse: result.rawResponse
        ? (result.rawResponse as unknown as Prisma.InputJsonValue)
        : Prisma.JsonNull,
      visualizations: Array.isArray(result.visualizations)
        ? (result.visualizations as Prisma.InputJsonValue)
        : Prisma.JsonNull,
    };
    const updated = id
  ...
  private async copyVisualizations(
    submissionId: string,
    analysisId: string,
    frontMediaId: string | null,
    backMediaId: string | null,
    ...
    for (const visualization of visualizations ?? []) {
      const sourceMediaId =
        visualization.side === 'FRONT' ? frontMediaId : backMediaId;
      if (!sourceMediaId) continue;
      for (const [type, url] of [
        ['overview', visualization.overviewUrl],
        ['centering', visualization.centeringUrl],
      ] as const) {
        ...
        try {
          ...
          if (existing?.sha256) {
            copied.push({
              ...
              centering: visualization.centering,
            });
            continue;
          }
          ...
          if (!stored?.magicMimeType?.startsWith('image/')) {
          ...
          /* Visualization is optional evidence; preserve the grade. */
        }
      }
    }
    return copied;
  ...
  private async decorateProjection(
    item: Prisma.RawCardPreGradeGetPayload<Prisma.RawCardPreGradeDefaultArgs>,
  ) {
    const projection = preGradeProjection(item);
    const visualizations = Array.isArray(item.visualizations)
      ? item.visualizations
      : [];
    projection.visualizations = await Promise.all(
      visualizations
        .filter(isPersistedVisualization)
...
function imageMimeTypeFromUrl(value: string) {
  try {
    const extension = new URL(value).pathname
      .toLowerCase()
      .match(/\.(avif|bmp|gif|jpe?g|png|tiff?|webp)$/)?.[1];
...
function isRawMetadata(value: Prisma.JsonValue | null) {
  ...
  const grader = String(metadata.grader ?? '')
    .trim()
    .toLowerCase();
...
export function preGradeProjection(
  item: Prisma.RawCardPreGradeGetPayload<Prisma.RawCardPreGradeDefaultArgs>,
) {
  return {
    id: item.id,
...
function isTrustedVisualizationUrl(value: string) {
  try {
    const url = new URL(value);
    return (
      url.protocol === 'https:' &&
      ['s3-eu-west-1.amazonaws.com', 's3-eu-central-1.amazonaws.com'].includes(
        url.hostname,
      )
    );
  } catch {
