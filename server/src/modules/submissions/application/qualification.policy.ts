...
export type QualificationCheckResult =
  'PASS' | 'ADVISORY' | 'UNCERTAIN' | 'FAIL' | 'ACTION_REQUIRED' | 'BLOCKED';
...
export type QualificationPolicyEnvironment =
...
/**
 * Policies are deliberately isolated by application environment. A legacy
 * global `default` row must never let production controls turn off staging
 * automation (or vice versa).
 */
export function qualificationPolicyKey(
  environment: QualificationPolicyEnvironment,
) {
  return `environment:${environment}`;
}

/**
 * Production requires an administrator to explicitly enable automation. Test,
 * beta, and development create an enabled policy when no environment policy
 * has been stored yet, while still respecting any explicit disable or kill
 * switch stored for that environment.
 */
export function defaultQualificationPolicyForEnvironment(
  environment: QualificationPolicyEnvironment,
) {
  return {
...
export function calculateProvisionalTerms(
...
export function isPolicyEligibleCategory(
  category: string,
  policy: QualificationPolicy,
) {
  return (
    policy.enabledCategories.length === 0 ||
...
export function qualificationCustomerStatus(outcome: QualificationOutcome) {
  switch (outcome) {
    case 'AUTO_QUALIFIED':
      return 'PRE_SALE_QUALIFIED';
    case 'COLLECTOR_ACTION_REQUIRED':
      return 'NEEDS_YOUR_ACTION';
    case 'BLOCKED':
      return 'BLOCKED_CONTACT_SUPPORT';
    default:
...
export function evaluateQualification(input: {
  ...
  if (['SUSPENDED', 'CLOSED', 'DEACTIVATED'].includes(input.accountStatus))
    add(
      'ACCOUNT_STATUS',
      'BLOCKED',
      'The collector account is not eligible to submit this listing.',
    );
  else if (input.accountStatus !== 'ACTIVE')
    add(
      'ACCOUNT_STATUS',
      'UNCERTAIN',
      'The collector account requires staff review.',
    );
  const requiredIdentity = ['name', 'year', 'set', 'cardNumber'];
  const missingIdentity = requiredIdentity.filter(
    (key) => !String(input.identity[key] ?? '').trim(),
  );
  add(
    'IDENTITY_COMPLETE',
    missingIdentity.length ? 'ACTION_REQUIRED' : 'PASS',
    missingIdentity.length
      ? `Add: ${missingIdentity.join(', ')}.`
      : 'The collectible identity is complete.',
    true,
    { missing: missingIdentity },
  );
  const requiredSlots = raw
    ? ['front', 'back']
    : ['front', 'back', 'grading-label'];
  const safeSlots = new Set(
    input.media
      .filter(
        (media) =>
          media.status === 'SAFE' && !media.deletedAt && media.sizeBytes > 0,
      )
      .map((media) => media.slot),
  );
  const missingMedia = requiredSlots.filter((slot) => !safeSlots.has(slot));
  add(
    'EVIDENCE_COMPLETE',
    missingMedia.length ? 'ACTION_REQUIRED' : 'PASS',
    missingMedia.length
      ? `Upload safe evidence for: ${missingMedia.join(', ')}.`
      : 'Required evidence is present and safe.',
    true,
    { missing: missingMedia },
  );
  add(
    'POSSESSION_CONFIRMED',
    input.possession ? 'PASS' : 'ACTION_REQUIRED',
    input.possession
      ? 'The collector confirmed possession.'
      : 'Confirm that you currently have the physical collectible.',
    true,
  );
  add(
    'INTAKE_DESTINATION',
    input.intakeValid ? 'PASS' : 'ACTION_REQUIRED',
    input.intakeValid
      ? 'The selected intake destination is eligible.'
      : 'Select an eligible Slice intake destination and delivery method.',
    true,
  );
  if (raw)
    add(
      'ASSET_TYPE',
      'UNCERTAIN',
      'Raw or ungraded collectibles require manual review by policy.',
    );
  else if (!input.policy.enabledGraders.includes(input.grader.toUpperCase()))
    add(
      'GRADER_POLICY',
      'UNCERTAIN',
      `${input.grader} is not enabled for automated qualification.`,
    );
  else if (!isPolicyEligibleCategory(input.category, input.policy))
    add(
      'CATEGORY_POLICY',
      'UNCERTAIN',
      'This collectible category requires manual review by policy.',
    );
  else
    add(
      'GRADER_POLICY',
      'PASS',
      `${input.grader} is enabled for automated qualification.`,
    );
  if (!raw) {
  ...
  add(
    'PROVISIONAL_TERMS',
    terms ? 'PASS' : 'ACTION_REQUIRED',
    terms
      ? 'A valid provisional Pre-Sale allocation can be calculated.'
      : 'Add a valid expected value and offer percentage before submitting.',
    true,
  );
  add(
    'MARKET_REFERENCE',
    ['FOUND', 'LIMITED', 'NO_MATCHES', 'COMPLETED'].includes(
      String(input.marketState),
    )
      ? 'PASS'
      : 'ADVISORY',
    input.marketState
      ? `Market reference status: ${input.marketState}.`
      : 'No market reference was available; staff can review pricing separately.',
    false,
  );
  ...
  const outcome: QualificationOutcome = blocked
    ? 'BLOCKED'
    : action
      ? 'COLLECTOR_ACTION_REQUIRED'
      : uncertain
  ...
  return {
    ...
    checks,
    reasons: checks
      .filter((check) => check.result !== 'PASS' && check.result !== 'ADVISORY')
