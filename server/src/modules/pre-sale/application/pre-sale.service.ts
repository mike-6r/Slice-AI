...
export class PreSaleService {
  ...
  private async projectionForId(id: string, db: Db, admin = false) {
    const row = await db.preSale.findUnique({ where: { id }, include: { initialOffering: { include: { inventory: true } }, asset: { include: { submissions: { where: { status: 'APPROVED' }, orderBy: { updatedAt: 'desc' }, take: 1, include: { intake: { include: { shipment: true, receipt: true, verification: true, exceptions: { where: { resolvedAt: null } } } } } }, custodyRecord: true, tradingMarket: true } }, reservations: { orderBy: [{ createdAt: 'asc' }, { id: 'asc' }] }, auditEvents: { orderBy: [{ createdAt: 'desc' }, { id: 'desc' }], take: 50 } } });
    ...
    const finalizationBlockers = physical !== 'CUSTODY_ESTABLISHED'
      ? ['PHYSICAL_CUSTODY_REQUIRED']
      : active.length === 0
        ? []
        : [
            ...(!row.initialOffering.inventory ? ['OFFERING_INVENTORY_REQUIRED'] : []),
            ...(!(row.asset.tradingMarket?.status === 'OPEN' && row.asset.tradingMarket.tradingEnabled) ? ['MARKET_OPEN_REQUIRED'] : []),
          ];
    const nextStep = finalizationBlockers.includes('OFFERING_INVENTORY_REQUIRED')
