...
import { formatAuthoritativeMoney, formatDisplayMoney } from "./currency-presentation";
...
describe("currency presentation", () => {
  it("converts display-only GBP figures using the current rate snapshot", () => {
    expect(formatDisplayMoney("10000", "GBP", "USD", rates)).toContain("125");
    expect(formatDisplayMoney("10000", "GBP", "EUR", rates)).toContain("116");
  });

  it("keeps the original amount visible when conversion is unavailable", () => {
    expect(formatDisplayMoney("10000", "GBP", "CAD", null)).toContain("£100");
  });
