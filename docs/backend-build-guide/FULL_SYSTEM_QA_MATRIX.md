...
| 011 | Custody, valuation, coverage, publication | Staff-only lifecycle discovery, valuation, custody, coverage, readiness and publish controls are available through API-mode frontend; the named beta-only physical exception is separately guarded, audited and idempotent | Server permission/recent-auth/readiness authority remains controlling; public projections retain actual physical state | PASS |
