...
Run from `server` with the normal staging environment loaded:

```bash
npm run staging:demo:collector:refresh
```
...
- five D11-published marketplace listings backed by custody, valuation, coverage and publication services;
