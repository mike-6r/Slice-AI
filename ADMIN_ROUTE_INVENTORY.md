...
| `/admin?section=memberships` | Full-width directory; All, Active, Past Due, Trialing, Cancelling, Cancelled, Expired; search, plan/status/billing/usage filters, pagination, Manage | `GET /api/v1/admin/memberships` | Provider-owned actions are explicit disabled states when Beta billing is not configured | `admin.console.read` | Compact redesign implemented; authenticated staging and mutation retest required |
| `/admin?section=memberships&membership=:id` | Membership detail: Overview, Usage & Capacity, Entitlements, billing truth, warnings, Open account | Account-linked membership row from `GET /api/v1/admin/memberships` | No fake provider mutation; disabled with reason in Beta | `admin.console.read` | Implemented read-only detail; direct detail/API and mutation retest required |
