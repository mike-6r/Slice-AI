...
export class PrismaCommunityRepository {
  ...
  async createSuggestion(guildId: string, creatorDiscordUserId: string, content: string): Promise<Suggestion> { return this.prisma.$transaction(async (db) => { const latest = await db.discordSuggestion.findFirst({ where: { guildId }, orderBy: { referenceNumber: 'desc' }, select: { referenceNumber: true } }); return mapSuggestion(await db.discordSuggestion.create({ data: { guildId, creatorDiscordUserId, content, referenceNumber: (latest?.referenceNumber ?? 0) + 1 } })); }); }
  ...
  async pollVote(id: string, userId: string, optionIndex: number): Promise<Poll | null> { const poll = await this.poll(id); if (!poll || poll.status !== 'OPEN' || poll.closesAt && poll.closesAt <= new Date()) return null; await this.prisma.discordPollVote.upsert({ where: { pollId_discordUserId: { pollId: id, discordUserId: userId } }, create: { pollId: id, discordUserId: userId, optionIndex }, update: { optionIndex } }); return this.poll(id); }
  ...
  async closeDuePolls(): Promise<Poll[]> { const rows = await this.prisma.discordPoll.findMany({ where: { status: 'OPEN', closesAt: { lte: new Date() } } }); await this.prisma.discordPoll.updateMany({ where: { id: { in: rows.map((row) => row.id) } }, data: { status: 'CLOSED' } }); return rows.map((row) => mapPoll({ ...row, status: 'CLOSED' })); }
