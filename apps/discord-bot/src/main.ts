...
import { applyTemplate, builderControls, canManageEmbeds, EMBED_TEMPLATES, parseFields, parseLinkButtons, renderEmbed, validateEmbed, type EmbedPayload, type EmbedTemplate, type LinkButton } from './embed-builder.js';
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId.startsWith('slice:embed:')) return void await handleEmbedSelect(interaction);
...
async function handleEmbedButton(interaction: ButtonInteraction): Promise<void> {
  const draft = await activeEmbedDraft(interaction); if (!draft) return;
  const action = interaction.customId.split(':')[3];
  if (action === 'preview') return void await interaction.reply({ ephemeral: true, embeds: [renderEmbed(draft.payload)] });
  if (action === 'template') return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.info('Choose a starting template', 'Templates replace the current embed content. Link buttons and target are preserved.')], components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(new StringSelectMenuBuilder().setCustomId(`slice:embed:${draft.id}:template-select`).setPlaceholder('Choose a template').addOptions(Object.entries(EMBED_TEMPLATES).map(([value, template]) => ({ label: template.label, value }))))] });
  if (action === 'publish') { const errors = validateEmbed(draft.payload, draft.buttons); if (errors.length || !draft.targetChannelId) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Needs attention', errors[0] ?? 'Choose a target channel before publishing.')] }); try { const published = await publishEmbed({ client, guildId: draft.guildId, draft, actorId: interaction.user.id, channelId: draft.targetChannelId, payload: draft.payload, buttons: draft.buttons, repository: embeds }); return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.success('Embed published', `Published to <#${published.channelId}> with a durable receipt.`)] }); } catch { return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Publication unavailable', 'The target is unavailable or Discord did not confirm publication.')] }); } }
  const modal = embedModal(draft.id, action); if (modal) return void await interaction.showModal(modal);
}

async function handleEmbedSelect(interaction: StringSelectMenuInteraction): Promise<void> {
  const draft = await activeEmbedDraft(interaction); if (!draft || interaction.customId.split(':')[3] !== 'template-select') return;
  const template = interaction.values[0] as EmbedTemplate; if (!Object.hasOwn(EMBED_TEMPLATES, template)) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Template unavailable', 'Choose a listed template.')] });
  const saved = await embeds.save(draft.id, draft.guildId, interaction.user.id, draft.version, { payload: applyTemplate(template), buttons: draft.buttons, targetChannelId: draft.targetChannelId });
  if (!saved) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.warning('Draft changed', 'Reopen the builder before applying a template.')] });
  await interaction.update({ embeds: [embedDashboard(saved)], components: builderControls(saved.id) });
}

async function handleEmbedModal(interaction: ModalSubmitInteraction): Promise<void> {
  const draft = await activeEmbedDraft(interaction); if (!draft) return; const action = interaction.customId.split(':')[3];
  if (action === 'schedule') return void await scheduleEmbedDraft(interaction, draft);
  const field = (key: string) => interaction.fields.getTextInputValue(key).trim(); let payload: EmbedPayload = { ...draft.payload }; let buttons: LinkButton[] = draft.buttons; let target = draft.targetChannelId;
  if (action === 'title') payload.title = field('value') || undefined;
  else if (action === 'description') payload.description = field('value') || undefined;
  else if (action === 'color') { const value = field('value'); if (!/^#[0-9a-f]{6}$/i.test(value)) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Invalid color', 'Use #RRGGBB.')] }); payload.color = Number.parseInt(value.slice(1), 16); }
  else if (action === 'author') { const name = field('name'); payload.author = name ? { name, ...(field('icon') ? { iconUrl: field('icon') } : {}), ...(field('url') ? { url: field('url') } : {}) } : undefined; }
  else if (action === 'footer') { const text = field('text'); payload.footer = text ? { text, ...(field('icon') ? { iconUrl: field('icon') } : {}) } : null; payload.timestamp = ['yes', 'true', 'on'].includes(field('timestamp').toLowerCase()); }
  else if (action === 'media') { payload = { ...payload, image: field('image') || undefined, thumbnail: field('thumbnail') || undefined, url: field('url') || undefined }; }
  else if (action === 'fields') { const parsed = parseFields(field('value')); if (!parsed) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Invalid fields', 'Use one field per line: Name | value | inline (optional).')] }); payload.fields = parsed; }
  else if (action === 'buttons') { const parsed = parseLinkButtons(field('value')); if (!parsed) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Invalid links', 'Use one link per line: Label | https://url | optional emoji.')] }); buttons = parsed; }
  else if (action === 'target') { const channel = field('channel'); if (channel && !/^\d{17,20}$/.test(channel)) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Invalid channel ID', 'Paste a Discord text-channel ID, or leave it blank to clear the target.')] }); target = channel || null; }
  else return;
  const errors = validateEmbed(payload, buttons); if (errors.length) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Invalid embed', errors[0]!)] });
  const saved = await embeds.save(draft.id, draft.guildId, interaction.user.id, draft.version, { payload, buttons, targetChannelId: target }); if (!saved) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.warning('Draft changed', 'Reopen the builder before saving.')] });
  await interaction.reply({ ephemeral: true, embeds: [embedDashboard(saved)], components: builderControls(saved.id) });
}

async function activeEmbedDraft(interaction: ButtonInteraction | StringSelectMenuInteraction | ModalSubmitInteraction) { const id = interaction.customId.split(':')[2]; const session = id ? embedSessions.get(id) : undefined; if (!session || session.actorId !== interaction.user.id || session.guildId !== interaction.guildId || session.expiresAt < Date.now() || !canManageEmbeds(interaction)) { await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Builder unavailable', 'This private builder session has expired or belongs to another staff member.')] }); return null; } const draft = await embeds.get(session.draftId, session.guildId); if (!draft) { await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Draft unavailable', 'This draft is unavailable.')] }); return null; } return draft; }
function embedModal(id: string, action: string): ModalBuilder | null { const modal = new ModalBuilder().setCustomId(`slice:embed:${id}:${action}:modal`); if (action === 'title' || action === 'description' || action === 'color') return modal.setTitle(`Edit ${action}`).addComponents(row('value', action === 'description' ? 'Description' : action === 'color' ? 'Hex color (#RRGGBB)' : 'Title', action === 'description' ? TextInputStyle.Paragraph : TextInputStyle.Short, false, action === 'description' ? 4000 : 256)); if (action === 'author') return modal.setTitle('Edit author').addComponents(row('name', 'Author name (blank removes)', TextInputStyle.Short, false, 256), row('icon', 'Author icon URL (optional)', TextInputStyle.Short, false, 400), row('url', 'Author link URL (optional)', TextInputStyle.Short, false, 400)); if (action === 'footer') return modal.setTitle('Edit footer').addComponents(row('text', 'Footer text (blank removes)', TextInputStyle.Short, false, 2048), row('icon', 'Footer icon URL (optional)', TextInputStyle.Short, false, 400), row('timestamp', 'Show timestamp? yes or no', TextInputStyle.Short, false, 5)); if (action === 'media') return modal.setTitle('Edit media').addComponents(row('image', 'Main image URL (optional)', TextInputStyle.Short, false, 400), row('thumbnail', 'Thumbnail URL (optional)', TextInputStyle.Short, false, 400), row('url', 'Title link URL (optional)', TextInputStyle.Short, false, 400)); if (action === 'fields') return modal.setTitle('Edit fields').addComponents(row('value', 'One per line: Name | value | inline', TextInputStyle.Paragraph, false, 1800)); if (action === 'buttons') return modal.setTitle('Edit link buttons').addComponents(row('value', 'One per line: Label | https://url | emoji', TextInputStyle.Paragraph, false, 1000)); if (action === 'target') return modal.setTitle('Set target channel').addComponents(row('channel', 'Discord text-channel ID (blank clears)', TextInputStyle.Short, false, 32)); if (action === 'schedule') return modal.setTitle('Schedule this embed').addComponents(row('name', 'Schedule name', TextInputStyle.Short, true, 100), row('type', 'ONE_TIME, DAILY, WEEKLY, MONTHLY, CUSTOM_WEEKDAYS', TextInputStyle.Short, true, 20), row('time', 'Local time (HH:mm)', TextInputStyle.Short, true, 5), row('timezone', 'IANA timezone (e.g. America/New_York)', TextInputStyle.Short, true, 64), row('details', 'Date YYYY-MM-DD, weekdays, or monthly day', TextInputStyle.Short, false, 64)); return null; }
async function scheduleEmbedDraft(interaction: ModalSubmitInteraction, draft: import('./persistence/embed-repository.js').Draft) { const type = interaction.fields.getTextInputValue('type').trim().toUpperCase() as ScheduleTiming['type']; const details = interaction.fields.getTextInputValue('details').trim(); const timing: ScheduleTiming = { type, localTime: interaction.fields.getTextInputValue('time').trim(), timezone: interaction.fields.getTextInputValue('timezone').trim(), ...(type === 'ONE_TIME' ? { date: details } : type === 'MONTHLY' ? { dayOfMonth: Number(details) } : { weekdays: parseWeekdays(details) }) }; const errors = validateTiming(timing); const channel = draft.targetChannelId && interaction.guild ? await publishableChannel(interaction.guild, draft.targetChannelId) : null; if (errors.length || !channel || validateEmbed(draft.payload, draft.buttons).length) return void await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.error('Schedule needs attention', errors[0] ?? 'The draft target or content is not publishable.')] }); const key = crypto.randomUUID().slice(0, 12); scheduleSessions.set(key, { actorId: interaction.user.id, guildId: draft.guildId, expiresAt: Date.now() + 10 * 60_000, input: { guildId: draft.guildId, draftId: draft.id, actorId: interaction.user.id, name: interaction.fields.getTextInputValue('name').trim(), timing, payloadMode: 'SNAPSHOT', payloadSnapshot: draft.payload, linkButtonsSnapshot: draft.buttons, targetChannelId: channel.id } }); await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.info('Confirm scheduled announcement', `**${interaction.fields.getTextInputValue('name').trim()}**\n${type} · ${timing.localTime} ${timing.timezone}\nTarget: <#${channel.id}>\nContent: Snapshot of revision ${draft.version}`)], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`slice:schedule:create:${key}:confirm`).setLabel('Confirm schedule').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`slice:schedule:create:${key}:cancel`).setLabel('Cancel').setStyle(ButtonStyle.Secondary))] }); }
function embedDashboard(draft: import('./persistence/embed-repository.js').Draft) { const status = validateEmbed(draft.payload, draft.buttons); return SliceEmbed.info('Embed Builder', `**${draft.name}** · revision ${draft.version}\nTitle: ${draft.payload.title ? 'Ready' : 'Optional'} · Description: ${draft.payload.description ? 'Ready' : 'Optional'} · Color: ${draft.payload.color !== undefined ? 'Custom' : 'Slice default'}\nAuthor: ${draft.payload.author ? 'Custom' : 'None'} · Footer: ${draft.payload.footer === null ? 'None' : draft.payload.footer ? 'Custom' : 'Slice default'} · Timestamp: ${draft.payload.timestamp ? 'On' : 'Off'}\nMedia: ${draft.payload.image ? 'Image' : 'None'}${draft.payload.thumbnail ? ' + thumbnail' : ''} · Fields: ${draft.payload.fields?.length ?? 0} · Links: ${draft.buttons.length}\nTarget: ${draft.targetChannelId ? `<#${draft.targetChannelId}>` : 'Not selected'}\n\n**Status:** ${status.length || !draft.targetChannelId ? `Needs attention — ${status[0] ?? 'choose a target channel'}` : 'Ready to publish or schedule'}`); }
