...
const links = new BackendSeamAccountLinkClient();
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    const safeMessage = error instanceof Error ? error.message.slice(0, 1200) : String(error).slice(0, 1200);
...
async function handleOnboardingButton(interaction: ButtonInteraction): Promise<void> { const action = interaction.customId.split(':')[2]; if (action === 'connect' || action === 'verify') { const response = await links.createLinkChallenge({ discordUserId: interaction.user.id, guildId: interaction.guildId ?? 'dm' }); await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.warning('Secure website handoff required', response.status === 'BACKEND_SEAM_REQUIRED' ? response.message : 'Continue securely on Slice.')] }); return; } if (action === 'faq') { await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.info('Slice FAQ', Object.entries(FAQ).slice(0, 6).map(([key, value]) => `**${key}** — ${value}`).join('\n\n'))] }); return; } await interaction.reply({ ephemeral: true, embeds: [SliceEmbed.info('Support', 'Choose a category in #create-a-ticket to open private support.')] }); }
