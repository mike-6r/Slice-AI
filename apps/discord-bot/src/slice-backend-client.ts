...
export type CollectorSpotlightEligibility = { eligible: boolean; isPublic?: true; linkedDiscordUserId?: string | null; collector?: Omit<PublicCollector, 'publishedListings'> };
...
export class SliceBackendClient {
  ...
  async marketSummary(): Promise<BackendResult<MarketSummary>> { return this.publicRead('/market/summary', marketSummary); }
  ...
  async collectorSpotlightEligibility(slug: string): Promise<BackendResult<CollectorSpotlightEligibility>> { return this.serviceRequest(`/discord/bot/collectors/${encodeURIComponent(slug)}/spotlight`, 'GET', undefined, collectorSpotlightEligibility); }
...
function marketSummary(value: unknown): MarketSummary | null { const row = object(value); const totalEstimatedMarketValue = row.totalEstimatedMarketValue === null ? null : money(row.totalEstimatedMarketValue); const volume24h = row.volume24h === null ? null : money(row.volume24h); const dataStatus = row.dataStatus; if (!['LIVE', 'DELAYED', 'DEMO', 'UNAVAILABLE'].includes(String(dataStatus)) || typeof row.source !== 'string' || (typeof row.asOf !== 'string' && row.asOf !== null) || typeof row.activeAssetCount !== 'number' || typeof row.collectorCount !== 'number' || (row.totalEstimatedMarketValue !== null && !totalEstimatedMarketValue) || (row.volume24h !== null && !volume24h)) return null; return { totalEstimatedMarketValue, volume24h, activeAssetCount: row.activeAssetCount, collectorCount: row.collectorCount, source: row.source, asOf: row.asOf, dataStatus: dataStatus as MarketSummary['dataStatus'] }; }
...
function collectorSpotlightEligibility(value: unknown): CollectorSpotlightEligibility | null { const row = object(value); if (typeof row.eligible !== 'boolean') return null; if (!row.eligible) return { eligible: false }; const collector = object(row.collector); if (row.isPublic !== true || typeof row.linkedDiscordUserId !== 'string' || typeof collector.slug !== 'string' || typeof collector.publishedListingCount !== 'number' || (typeof collector.headline !== 'string' && collector.headline !== null) || (typeof collector.specialism !== 'string' && collector.specialism !== null) || (typeof collector.displayName !== 'string' && collector.displayName !== null)) return null; return { eligible: true, isPublic: true, linkedDiscordUserId: row.linkedDiscordUserId, collector: { slug: collector.slug, headline: collector.headline as string | null, specialism: collector.specialism as string | null, displayName: collector.displayName as string | null, publishedListingCount: collector.publishedListingCount } }; }
