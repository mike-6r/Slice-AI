...
const resourceRole = z.object({ key: z.string().regex(/^[a-z0-9-]+$/), name: z.string().min(1).max(100), color, hoist: z.boolean().default(false), mentionable: z.boolean().default(false), staff: z.boolean().default(false), permissions: z.array(z.string()).default([]) });
