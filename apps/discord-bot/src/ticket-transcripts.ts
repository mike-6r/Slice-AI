...
export type TranscriptSource = LifecycleTicket & { safeSummary: string; intakeAnswers?: { fieldLabel: string; value: string }[]; events: { type: string; actorDiscordId?: string; metadata?: Record<string, unknown> | null; createdAt: Date }[] };
export type StaffTranscriptSource = TranscriptSource & { internalNotes: { authorDiscordUserId: string; content: string; createdAt: Date }[] };
...
export interface StaffTranscriptRepository { getStaffTranscriptSource(ticketId: string, guildId: string): Promise<StaffTranscriptSource | null>; getStaffTranscript(ticketId: string): Promise<TranscriptRecord | null>; saveStaffTranscript(record: TranscriptRecord, retryFailed: boolean): Promise<TranscriptRecord>; }
...
export class StaffTicketTranscriptService {
  constructor(private readonly repository: StaffTranscriptRepository) {}
  async generate(ticketId: string, guildId: string, history: TicketHistory, retryFailed = false): Promise<{ transcript: TranscriptRecord; reused: boolean }> {
    const ticket = await this.repository.getStaffTranscriptSource(ticketId, guildId); if (!ticket || ticket.status !== 'CLOSED') throw new Error('Closed ticket unavailable.');
    const existing = await this.repository.getStaffTranscript(ticket.id); if (existing && (existing.status !== 'FAILED' || !retryFailed)) return { transcript: existing, reused: true };
    try { const snapshot = await history.read(ticket); const record: TranscriptRecord = { ticketId: ticket.id, status: snapshot.partial ? 'PARTIAL' : 'COMPLETE', content: renderStaffTranscript(ticket, snapshot.messages, snapshot.partial), messageCount: snapshot.messages.length }; return { transcript: await this.repository.saveStaffTranscript(record, retryFailed), reused: false }; }
    catch { const record: TranscriptRecord = { ticketId: ticket.id, status: 'FAILED', content: `# Slice staff ticket transcript\n\nTicket: ${ticket.id}\n\nTranscript capture could not read Discord history.`, messageCount: 0, failureReason: 'Discord history unavailable' }; return { transcript: await this.repository.saveStaffTranscript(record, true), reused: false }; }
  }
}
...
export function renderTranscript(ticket: TranscriptSource, messages: TranscriptMessage[], partial: boolean): string {
  ...
  if (ticket.intakeAnswers?.length) { lines.push('', '## Intake'); for (const answer of ticket.intakeAnswers) lines.push(`- ${sanitizeTicketText(answer.fieldLabel)}: ${sanitizeTicketText(answer.value)}`); }
...
export function renderStaffTranscript(ticket: StaffTranscriptSource, messages: TranscriptMessage[], partial: boolean): string { const base = renderTranscript(ticket, messages, partial); const notes = ticket.internalNotes.length ? ticket.internalNotes.map((note) => `- ${note.createdAt.toISOString()} — <@${note.authorDiscordUserId}>\n  ${sanitizeTicketText(note.content)}`).join('\n') : '_No internal notes._'; return `${base}\n\n## Staff-only internal notes\n${notes}`.slice(0, 200_000); }
