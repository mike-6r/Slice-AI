import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ChannelType, EmbedBuilder, Guild, PermissionFlagsBits, PermissionsBitField, StringSelectMenuBuilder, type GuildBasedChannel, type Role } from 'discord.js';
...
export class SetupProvisioner {
  ...
  async inspect(guild: Guild): Promise<ReconciliationPlan> {
    await guild.roles.fetch(); await guild.channels.fetch(); const config = await this.repository.getGuildConfig(guild.id); const plan: ReconciliationPlan = { missingRoles: 0, missingCategories: 0, missingChannels: 0, renamed: 0, moved: 0, permissionDrift: 0, roleDrift: 0, separatorDrift: 0, staleReferences: 0, missingPanels: 0, ambiguous: [], updateAvailable: Boolean(config && config.setupVersion < SLICE_DISCORD_SETUP_VERSION), artworkMissing: 0 };
    for (const spec of ROLE_DEFINITIONS) this.inspectRole(guild, await this.repository.getResource(guild.id, 'ROLE', spec.key), spec, plan);
    await this.inspectRoleHierarchy(guild, plan);
  ...
  async apply(guild: Guild): Promise<{ created: number; updated: number; plan: ReconciliationPlan }> {
    const plan = await this.inspect(guild); if (plan.ambiguous.length) throw new AmbiguousManagedResourceError(plan.ambiguous); if ((plan.hierarchyDrift ?? 0) > 0) throw new RoleHierarchyError(plan.hierarchyDrift ?? 0); let created = 0; let updated = 0;
    await this.repository.transaction(async () => {
      ...
      for (const spec of CATEGORY_DEFINITIONS) { const result = await this.ensureCategory(guild, spec.key, spec.name); created += result.created; updated += result.updated; }
  ...
  private inspectRole(guild: Guild, record: ManagedResource | null, spec: (typeof ROLE_DEFINITIONS)[number], plan: ReconciliationPlan): void { const current = record ? guild.roles.cache.get(record.discordId) : undefined; if (current) { if (current.name !== spec.name) plan.renamed++; const expectedPermissions = new PermissionsBitField(spec.permissions ?? []).bitfield; if (current.color !== spec.color || current.hoist !== Boolean(spec.hoist) || current.mentionable !== Boolean(spec.mentionable) || current.permissions.bitfield !== expectedPermissions) plan.roleDrift++; if (spec.separator && current.members.size !== 0) plan.separatorDrift++; return; } if (record) plan.staleReferences++; const candidates = guild.roles.cache.filter((role) => role.name === spec.name); if (candidates.size > 1) plan.ambiguous.push(`role:${spec.name}`); else plan.missingRoles++; }
  private async inspectRoleHierarchy(guild: Guild, plan: ReconciliationPlan): Promise<void> {
    const botHighest = guild.members.me?.roles.highest.position;
    if (botHighest === undefined) return;
    const managed = await Promise.all(ROLE_DEFINITIONS.map(async (spec) => ({ spec, record: await this.repository.getResource(guild.id, 'ROLE', spec.key) })));
    const roles = managed.flatMap(({ spec, record }) => record ? [{ spec, role: guild.roles.cache.get(record.discordId), id: record.discordId }] : []);
    let drift = 0;
    for (const { role } of roles) if (role && role.position >= botHighest) drift++;
    const slice = roles.find(({ spec }) => spec.key === 'slice');
    if (slice && !guild.members.me?.roles.cache.has(slice.id)) drift++;
    const slicePosition = slice?.role?.position;
    if (slicePosition !== undefined) for (const { spec, role } of roles) if (spec.key !== 'slice' && role && role.position >= slicePosition) drift++;
    plan.hierarchyDrift = drift;
  }
  ...
  private async applyCategoryPermissions(category: GuildBasedChannel, guild: Guild, key: string): Promise<void> {
  ...
  private async applyChannelPermissions(channel: GuildBasedChannel, guild: Guild, spec: (typeof CHANNEL_DEFINITIONS)[number]): Promise<void> {
    ...
    if (['welcome', 'announcements', 'my-slice', 'market-feed', 'roles', 'collector-workspace', 'list-a-collectible', 'create-a-ticket'].includes(spec.key)) {
...
function panelEmbed(key: string): EmbedBuilder {
  ...
  return SliceEmbed.panel(panel);
}
function panelComponents(key: string) {
  const config = presentationConfig();
  if (key === 'verify') return [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId('slice:onboarding:verify').setLabel('Start human check').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('slice:onboarding:why-verify').setLabel('Why verify?').setStyle(ButtonStyle.Secondary))];
  if (key === 'welcome') return [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId('slice:onboarding:my-slice').setLabel('Open My Slice').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('slice:onboarding:marketplace').setLabel('Explore Market').setStyle(ButtonStyle.Secondary))];
  if (key === 'start-here') return [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId('slice:onboarding:connect').setLabel('Connect account').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('slice:onboarding:verify').setLabel('Verify Discord access').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('slice:onboarding:faq').setLabel('Open FAQ').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('slice:onboarding:support').setLabel('Get support').setStyle(ButtonStyle.Secondary))];
  if (key === 'my-slice') return [new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId('slice:onboarding:my-slice').setLabel('Open My Slice').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('slice:onboarding:portfolio').setLabel('Portfolio').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('slice:onboarding:orders').setLabel('Orders').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('slice:onboarding:membership').setLabel('Membership').setStyle(ButtonStyle.Secondary))];
  ...
  if (key === 'create-a-ticket') { const tickets = config['tickets.yml']; const allowed = tickets.support_panel?.categories ?? tickets.categories.map((category) => category.key); const categories = allowed.flatMap((categoryKey) => tickets.categories.filter((category) => category.key === categoryKey)); return [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(new StringSelectMenuBuilder().setCustomId('slice:ticket:create').setPlaceholder(tickets.support_panel?.placeholder ?? 'Choose what you need help with').addOptions(categories.map((category) => ({ label: category.label, value: category.key, description: category.description, emoji: category.emoji }))))]; }
